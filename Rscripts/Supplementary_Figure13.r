...
refg_g<-read.table("../data/polyA_site_detection/araport_genes_and_polyA_gonly.bed",header=F,fill=TRUE);colnames(refg_g)[10]<-'ID';head(refg_g)
# set TTS of the gene
refg_g$genes_3p<-NA
...
# dataset1 ------------------
filelist<-list.files(path='../data/polyA_site_detection/polyA_dataset1/',pattern='bed',full.names = TRUE);filelist
RNA8list<-list()
...
# datasets2 ------------------
filelist<-list.files(path='../data/polyA_site_detection/polyA_dataset2/',full.names = TRUE,pattern='109');filelist
vernRNA<-data.frame()
for(i in 1:length(filelist)){
  R<-read.table(filelist[i],header=F)
  C<-R[-grep('t',R$V1),]
...
filelist<-list.files(path='../data/polyA_site_detection/polyA_dataset2/',full.names = TRUE,pattern='122');filelist
vernRNA<-data.frame()
for(i in 1:length(filelist)){
  R<-read.table(filelist[i],header=F)
  C<-R[-grep('t',R$V1),]
