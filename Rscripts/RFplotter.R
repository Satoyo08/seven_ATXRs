...
# plot bars
plot_RFmodel<-function(model,palette,sort,col_key,space){
  IMPs<-apply(model$Importance,1,mean)[sort];sds<-apply(model$Importance,1,sd)[sort]
  bp<-barplot(IMPs,col=palette[col_key],space=space,names.arg=F,ylim=c(0,max(IMPs+max(sds))));lims<-par('usr')
  plot(0,type='n',axes=FALSE,ann=FALSE,xlim=lims[1:2]+c(4,0),ylim=lims[3:4])
  bp<-barplot(IMPs,col=palette[col_key],space=space,names.arg=F,ylim=c(0,max(IMPs+max(sds))),add=T)
...
myROCplot<-function(model,error_FPR_position,ref){
  for(i in c(1:5)*2){
  ...
  text(0.7,0.1,paste("AUC = ",round(mean(auc),digits=3)))
