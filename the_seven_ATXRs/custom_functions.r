...
#                               bg      , A       ,   B     , A&B     , C       , A&C     , B&C     , ABC
rgb_pal<-c(makeTransparent('gray80',150),'#FF5E5B','#FFED66','#FFA661','#00CECB','#809693','#80DE99','#C09E7A') 
pgy_pal<-c(makeTransparent('gray80',150),'#F46D43','#3288BD','#E6F598','#5E4FA2','#ABDDA4','#2D004B','black') 
# dataframe preps
...
# makind graphs
plot_shu<-function(x,y,main='',col=rgb(0,0,0,0.3),pch=16,lwd=1,type='p',limmaxx=0.98,limmaxy=0.98,limminx=0,limminy=0){#quantile 0.98で軸を絞ってplot
  plot(x,y,xlim=c(quantile(x,limminx),quantile(x,limmaxx)),ylim=c(quantile(y,limminy),quantile(y,limmaxy)),pch=pch,col=col,xlab="",ylab="",lwd=lwd,type=type,main=main)
}
...
## definitions of ATX12R7-marked, ATX345-marked, ATXR3-marked genes. Same list with Oya.S et.al 2021.
RPKM<-read.table("../data/refs/ChIP1_RPKM.txt",header=TRUE)
RPM<-read.table("../data/refs/ChIP1_RPM.txt",header=TRUE)
